...
void f_log_f2b(enum e_filetobuf_error ret, const char* path, unsigned int buflen, unsigned int flen) {
...
void f_log_shader(enum e_gl_shader_check ret, int type, char* logbuf, unsigned int loglen) {
	const char* typestr;
	switch(type) {
...
void f_render_main(void* win) {
	...
	char* prog_log = chbuf + 2*BUFSZ_SRC + 3*BUFSZ_LOG;
	...
	f_log_f2b(ret, path, BUFSZ_SRC, len);

	unsigned int vert = 0;
	ret = f_gl_genshader(&vert, GL_VERTEX_SHADER, vert_src, vert_log, BUFSZ_LOG, &len);
	f_log_shader(ret, GL_VERTEX_SHADER, vert_log, len);
